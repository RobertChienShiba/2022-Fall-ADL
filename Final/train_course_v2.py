...
data['courses_v1'] = data['courses_v1'][data['courses_v1'].columns.drop(["course_price", "desc_embed", "upload_days"]).to_list() + ["course_price", "upload_days"]]
...
meta_info_len = len(data['users_v1'].columns) + 2

def make_dataset(name, remove_from=None, selected_courses=None, sample_course_sim=True, sample_course_idx=676):
    user_courses_id = []
    gid = []
    ...
    for idx, row in tqdm(data[name].iterrows()):
        ...
        x[cnt:cnt+user_course_len, 98:100] = course_arr[user_courses_id[idx]]
        x[cnt:cnt+user_course_len, 100:] = desc_embed_reduced[user_courses_id[idx]]
...
X_train, Y_train, gid_train, course_id_train = make_dataset("train", None, set([data['course2idx'][i] for i in data['courses'].course_id]), True, 676)

# X_val_seen, Y_val_seen, gid_val_seen, course_id_val_seen = make_dataset('val_seen', 'train', set([data['course2idx'][i] for i in data['courses'].query("val_seen == True").course_id]), 61)
...
X_test_unseen, Y_test_unseen, gid_test_unseen, course_id_test_unseen = make_dataset('test_unseen', None, set([data['course2idx'][i] for i in data['courses'].query("test_unseen == True").course_id]), False)
...
model = lgb.train(params, lgb_train, num_boost_round=100, valid_sets=[lgb_eval_unseen, lgb_train], valid_names=["unseen", "train"], feval=_acc_metric, callbacks=[lgb.log_evaluation(period=5)])

model.save_model("course_model.txt")
